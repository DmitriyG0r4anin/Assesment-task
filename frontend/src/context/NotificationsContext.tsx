...
import { pushMotionEvent } from '../lib/motion-events';
...
function parseMotionPayload(data: unknown): MotionEvent | null {
    ...
    const roomName = (raw.roomName as string) ?? (raw.RoomName as string) ?? undefined;
    if (!roomName) return null;
    const isDetected = (raw.isDetected as boolean) ?? (raw.IsDetected as boolean) ?? false;
    let timestamp = (raw.timestamp as string) ?? (raw.Timestamp as string) ?? undefined;
    if (timestamp) {
...
function handleMotionEvent(event: MotionEvent): void {
    ...
    toast.info(event.isDetected ? `Motion: ${event.roomName}` : `Clear: ${event.roomName}`, {
...
export function NotificationsProvider({ children }: Readonly<{ children: ReactNode }>) {
    ...
    useEffect(() => {
        ...
        void startConnection(connection, () => disposedRef.current).then(() => {
            if (!disposedRef.current && connection.state === HubConnectionState.Connected) {
                setConnectionStatus('connected');
            }
        });
