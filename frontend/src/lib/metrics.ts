...
export function sortByTimestampDesc<T extends { timestamp: string }>(
  items: T[],
): T[] {
  return [...items].sort(
...
export function sortByTimestampAsc<T extends { timestamp: string }>(
  items: T[],
): T[] {
  return [...items].sort(
...
export function buildRoomNameMap(
...
export function refreshIntervalToMs(
  value: number,
...
const LARGER_REFRESH_UNIT: Record<
  RefreshIntervalUnit,
  RefreshIntervalUnit | null
> = {
  seconds: "minutes",
  minutes: "hours",
  hours: null,
...
const SMALLER_REFRESH_UNIT: Record<
  RefreshIntervalUnit,
  RefreshIntervalUnit | null
> = {
  seconds: null,
